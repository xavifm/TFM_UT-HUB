...
void AWorldManager::LoadPortion(int _index)
{
	...
	if (cameraActor) 
	{
		UCameraComponent* minigameCamera = cameraActor->GetCameraComponent();
		minigameCamera->Activate();
		PC->SetViewTargetWithBlend(minigameCamera->GetAttachParentActor(), 0.0f);
	}
}
